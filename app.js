...
function renderMonetization() {
  ...
  els.monetizationIdeas.innerHTML = ideas.length ? ideas.map(idea => `
  ...
  els.monetizationLog.innerHTML = log.length ? `<h4>Delivery log</h4><ul>${log.slice(-5).reverse().map(l => `<li><time>${l.delivered_at ? l.delivered_at.split('T')[0] : '—'}</time>: ${escapeHtml(l.idea_title || l.idea_id || '—')}</li>`).join('')}</ul>` : '';
}
